  ...
  const teamMembers = [
    {
      ...
      color: "#F59E0B" // Changed to yellow
    },
    {
      name: "ANIRUDDH SINGH",
  ...
  return (
    <section className="relative z-10 py-32 overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute -top-48 -left-48 w-96 h-96 bg-yellow-600/5 rounded-full blur-3xl"></div>
        ...
        {/* Floating elements */}
        <div className="absolute top-1/3 left-1/5 w-3 h-3 bg-yellow-400 rounded-full animate-float opacity-60"></div>
        <div className="absolute top-1/2 right-1/4 w-4 h-4 bg-amber-400 rounded-full animate-float opacity-60 delay-200"></div>
        <div className="absolute bottom-1/3 left-1/4 w-5 h-5 bg-yellow-400 rounded-full animate-float opacity-60 delay-400"></div>
      </div>
        ...
        <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="flex justify-center gap-4 mt-8">
            <div className="flex items-center bg-yellow-900/30 backdrop-blur-md px-6 py-3 rounded-full border border-yellow-500/20">
              ...
              <span className="text-yellow-200">Creative Thinking</span>
            </div>
            <div className="flex items-center bg-yellow-900/30 backdrop-blur-md px-6 py-3 rounded-full border border-yellow-500/20">
              <div className="w-3 h-3 bg-amber-400 rounded-full mr-2 animate-pulse"></div>
              <span className="text-yellow-200">Data-Driven Execution</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className={`transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100' : 'opacity-0 translate-y-10'}`}>
          <div className="relative">
            {/* Glow effect */}
            <div className="absolute -inset-1 bg-gradient-to-r from-yellow-600 to-amber-600 rounded-2xl blur opacity-20"></div>
            
            {/* Content box */}
            <div className="relative bg-gradient-to-br from-yellow-900/40 to-amber-900/20 backdrop-blur-md rounded-xl border border-yellow-500/20 overflow-hidden shadow-lg shadow-yellow-900/10 p-8">
              {/* Tabs */}
              <div className="flex mb-8 bg-yellow-900/30 rounded-full p-1">
              ...
              <div className="min-h-48">
                {activeTab === 'mission' && (
                  <div className="animate-fade-in">
                    <h3 className="text-2xl font-bold text-yellow-100 mb-4">Our Mission</h3>
                ...
                {activeTab === 'vision' && (
                  <div className="animate-fade-in">
                    <h3 className="text-2xl font-bold text-yellow-100 mb-4">Our Vision</h3>
                    <p className="text-yellow-200 mb-4">
                ...
                {activeTab === 'approach' && (
                  <div className="animate-fade-in">
                    <h3 className="text-2xl font-bold text-yellow-100 mb-4">Our Approach</h3>
                    <p className="text-yellow-200 mb-4">
      ...
      <div className="max-w-7xl mx-auto mt-24 px-4">
        <h3 className="text-3xl font-bold text-center text-yellow-100 mb-12">Meet Our Leadership</h3>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {teamMembers.map((member, index) => (
            <div 
              key={index} 
              className={`bg-gradient-to-br from-yellow-900/30 to-amber-900/10 backdrop-blur-md rounded-xl border border-yellow-500/20 overflow-hidden shadow-lg shadow-yellow-900/10 group hover:shadow-yellow-500/30 hover:border-yellow-400/40 hover:scale-105 transition-all duration-500 animate-fade-in delay-${index * 100}`}
            >
              ...
              <div className="p-4">
                <h4 className="text-lg font-bold text-yellow-100">{member.name}</h4>
                <p className="text-yellow-300 text-sm">{member.position}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Values section */}
      <div className="max-w-7xl mx-auto mt-24 px-4">
        <h3 className="text-3xl font-bold text-center text-yellow-100 mb-12">Our Core Values</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {companyValues.map((value, index) => (
            <div 
              key={index} 
              className={`bg-gradient-to-br from-yellow-900/20 to-amber-900/10 backdrop-blur-md rounded-xl border border-yellow-500/20 p-6 group hover:shadow-yellow-500/30 hover:border-yellow-400/40 transition-all duration-500 animate-fade-in delay-${index * 100}`}
            >
              <div 
                className="bg-gradient-to-br from-yellow-900/60 to-yellow-900/30 p-4 rounded-full text-yellow-200 inline-block group-hover:scale-110 transition-all duration-500 mb-4"
                style={{ color: value.color }}
              >
                {value.icon}
              </div>
              
              <h4 className="text-xl font-bold text-yellow-100 mb-2">{value.title}</h4>
              <p className="text-yellow-300">{value.description}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto mt-24 px-4 text-center">
        <div className="relative">
          ...
          {/* Content box */}
          <div className="relative bg-gradient-to-br from-yellow-900/40 to-amber-900/20 backdrop-blur-md rounded-xl border border-yellow-500/20 overflow-hidden shadow-lg shadow-yellow-900/10 p-8">
            <h3 className="text-3xl font-bold text-yellow-100 mb-4">Ready to Transform Your Digital Presence?</h3>
      ...
      <div className="max-w-7xl mx-auto mt-24 px-4 text-center">
        <div className="flex items-center justify-center gap-2">
          ...
          <p className="text-yellow-400 text-sm">Backed by Midnight Solutions Pvt. Ltd.</p>
        </div>
      </div>
    </section>
  );
